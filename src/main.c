...
static struct calibration_points calib_points = {.screen_x = {64, 20, 108, 108, 20},
...
static const struct device *const touch_dev = DEVICE_DT_GET(DT_NODELABEL(tsc20030));
...
void calculate_calibration_coefficients(void)
{
	...
	for (int i = 0; i < CALIBRATION_POINTS; i++) {
		...
		sum_raw_x_screen_x =
			sum_raw_x_screen_x + calib_points.raw_x[i] * calib_points.screen_x[i];
		sum_raw_y_screen_y =
			sum_raw_y_screen_y + calib_points.raw_y[i] * calib_points.screen_y[i];
		sum_raw_x_squared =
			sum_raw_x_squared + calib_points.raw_x[i] * calib_points.raw_x[i];
		sum_raw_y_squared =
			sum_raw_y_squared + calib_points.raw_y[i] * calib_points.raw_y[i];
	}
	...
	cal_coeff_x.a = (CALIBRATION_POINTS * sum_raw_x_screen_x - sum_raw_x * sum_screen_x) /
			denominator_x;
	cal_coeff_x.b =
		(sum_screen_x * sum_raw_x_squared - sum_raw_x * sum_raw_x_screen_x) / denominator_x;

	cal_coeff_y.a = (CALIBRATION_POINTS * sum_raw_y_screen_y - sum_raw_y * sum_screen_y) /
			denominator_y;
	cal_coeff_y.b =
		(sum_screen_y * sum_raw_y_squared - sum_raw_y * sum_raw_y_screen_y) / denominator_y;
...
static void button_pressed(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
	...
	draw_calibration_marker(calib_points.screen_x[point_index],
				calib_points.screen_y[point_index]);
}
